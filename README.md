...
サーバ上の Docker Compose アプリケーションを Web UI と MCP 経由で管理するツール。
...
```bash
# 必要な環境変数を設定して起動
export CORS_ALLOWED_ORIGINS="https://docker-manager.example.com"
export COOKIE_SECURE=true   # HTTPS 経由なら必須
docker compose up -d --build
```

DB ファイル (`/data/docker-manager.db`) は `dm-data` ボリュームに保存される。バックアップは このボリュームの中身をコピーすればよい。
...
| `DB_PATH`              | `./data/docker-manager.db` (dev), `/data/docker-manager.db` | SQLite ファイルパス                                                                        |
...
- セッションは cookie ベース（`dm_session`, HttpOnly, SameSite=Lax）
...
- UI で発行した MCP トークン (`dmt_...`) を `Authorization: Bearer <token>` で送信
...
```json
{
  "mcpServers": {
    "docker-manager": {
      ...
      "headers": { "Authorization": "Bearer dmt_xxxxxxxx..." }
    }
  }
}
```

Claude Code を再起動して MCP server を承認すると `mcp__docker-manager__*` ツールが利用できる。
